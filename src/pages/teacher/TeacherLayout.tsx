    ...
    <DashboardShell roleLabel="Guro" hideHeader bgImage={teacherBg}>
      <div className="flex min-h-screen min-w-0">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-[var(--color-border)] bg-[var(--color-surface)]/95 px-4 shadow-sm backdrop-blur lg:hidden"><div className="flex items-center gap-2"><img src={logo} alt="LinawLetra" className="h-10 w-10 rounded-xl object-cover" /><span className="font-bold text-[var(--color-brand-sage)]">Teacher Panel</span></div><div className="flex items-center gap-2"><ProfileMenu collapsed mobile /><button type="button" onClick={() => setMobileOpen((value) => !value)} aria-expanded={mobileOpen} className="flex h-12 min-w-12 items-center justify-center rounded-2xl border border-[var(--color-border)] bg-white/75"><AppIcon name="☰" /><span className="sr-only">Menu</span></button></div></header>
