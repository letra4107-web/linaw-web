  ...
  return (
    <div ref={menuRef} className="relative min-w-0">
      <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-haspopup="menu" className={`flex min-h-12 w-full min-w-0 items-center gap-3 rounded-2xl border p-2 text-left transition-colors ${mobile ? 'border-[var(--color-border)] bg-white/75' : 'border-white/20 bg-white/10 text-white hover:bg-white/15'} ${collapsed ? 'justify-center' : ''}`}>
        ...
        {!collapsed && <span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold">{name}</span><span className={`block text-xs ${mobile ? 'text-[var(--color-text-muted)]' : 'text-white/65'}`}>Account ng guro</span></span>}
      </button>
      {open && <div role="menu" className={`absolute z-50 w-72 max-w-[calc(100vw-2rem)] rounded-3xl border p-3 shadow-raised ${mobile ? 'top-full right-0 mt-2' : 'bottom-full left-0 mb-2'}`} style={cardStyle('--color-brand-sage', 8, 35)}><div className="mb-2 flex items-center gap-3 rounded-2xl bg-white/70 p-3"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[var(--color-brand-sage)] font-bold text-white">{initials}</span><div className="min-w-0"><p className="truncate font-bold">{name}</p><p className="truncate text-xs text-[var(--color-text-muted)]">{user?.email}</p></div></div><NavLink to="/teacher/settings" onClick={() => setOpen(false)} role="menuitem" className="flex min-h-11 items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold hover:bg-white/70"><AppIcon name="⚙" className="h-4 w-4" /> Profile at Mga Setting</NavLink><div className="my-2 border-t border-white/70" /><button type="button" onClick={() => supabase.auth.signOut()} role="menuitem" className="flex min-h-11 w-full items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold text-[var(--color-danger)] hover:bg-[var(--color-danger-soft)]"><AppIcon name="↩" className="h-4 w-4" /> Mag-sign out</button></div>}
    </div>
  );
}
...
export default function TeacherLayout() {
  ...
  return (
    <DashboardShell roleLabel="Guro" hideHeader roleTheme="teacher">
      <div className="flex min-h-screen min-w-0">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          {mobileOpen && <div className="sticky top-16 z-30 border-b border-white/15 bg-[var(--color-primary-hover)] p-3 shadow-card lg:hidden"><nav className="grid grid-cols-2 gap-2 sm:grid-cols-3">{PRIMARY_TABS.map((tab) => <NavItem key={tab.to} {...tab} collapsed={false} onNavigate={() => setMobileOpen(false)} />)}</nav></div>}
