...
export default function Dashboard() {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6">
      ...
      <section aria-labelledby="recent-title" className="rounded-3xl border p-5 shadow-card sm:p-6" style={cardStyle('--color-brand-teal', 7, 26)}><div className="flex items-center justify-between"><h2 id="recent-title" className="text-xl font-bold">Kamakailang Aktibidad</h2><span className="text-xs font-bold text-[var(--color-text-muted)]">Pinakabagong practice</span></div>{recent?.length ? <div className="mt-4 grid grid-cols-1 gap-2 md:grid-cols-2">{recent.map((session, index) => <div key={`${session.created_at}-${index}`} className="flex min-w-0 items-center gap-3 rounded-2xl border border-white/70 bg-white/65 p-3"><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl font-bold ${session.accuracy_percentage >= 80 ? 'bg-[var(--color-success-soft)] text-[var(--color-success)]' : 'bg-[var(--color-warning-soft)] text-[var(--color-warning-text)]'}`}><AppIcon name={session.accuracy_percentage >= 80 ? '✓' : '⚠'} className="h-5 w-5" /></span><div className="min-w-0 flex-1"><p className="truncate font-bold">{nameFor(session.student_id)} · {session.word}</p><time dateTime={session.created_at} className="text-xs text-[var(--color-text-muted)]">{new Date(session.created_at).toLocaleString('fil-PH', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}</time></div><span className="font-bold">{session.accuracy_percentage}%</span></div>)}</div> : <p className="mt-4 text-[var(--color-text-muted)]">Wala pang recent activity.</p>}</section>
