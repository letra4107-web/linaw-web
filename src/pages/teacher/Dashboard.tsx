...
export default function Dashboard() {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6">
      <header className="relative overflow-hidden rounded-3xl border px-5 py-5 text-white shadow-card sm:px-7" style={{ backgroundImage: 'linear-gradient(135deg, #5c8047, #0d9488)' }}><div aria-hidden="true" className="absolute -top-16 -right-12 h-44 w-44 rounded-full bg-white/10" /><div className="relative"><p className="text-sm font-bold text-white/75">Buod ng guro</p><h1 className="text-2xl font-bold sm:text-3xl">Magandang araw, {identity?.displayName ?? 'Guro'}</h1><p className="mt-1 text-sm text-white/85">Narito ang pinakamahalagang update sa iyong klase.</p></div></header>
      ...
      <div className="grid min-w-0 grid-cols-1 gap-5 lg:grid-cols-[1.1fr_0.9fr]">
        <section aria-labelledby="attention-title" className="rounded-3xl border p-5 shadow-card sm:p-6" style={cardStyle('--color-brand-coral', 7, 28)}><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold tracking-[0.1em] text-[var(--color-brand-coral)] uppercase">Prayoridad</p><h2 id="attention-title" className="text-xl font-bold">Mga Mag-aaral na Kailangang Tutukan</h2></div><Link to="/teacher/progress-reports" className="text-sm font-bold text-[var(--color-primary)]">Pagsusuri →</Link></div>
          {needsAttention.length ? <ul className="mt-4 flex flex-col gap-2">{needsAttention.slice(0, 5).map((student) => <li key={student.child_id} className="flex items-center gap-3 rounded-2xl border border-white/70 bg-white/65 p-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--color-danger-soft)] font-bold text-[var(--color-danger)]">{student.name.charAt(0)}</span><span className="min-w-0 flex-1"><span className="block truncate font-bold">{student.name}</span><span className="block text-xs text-[var(--color-text-muted)]">{student.activities_completed} gawaing natapos</span></span><span className="rounded-full bg-[var(--color-danger-soft)] px-3 py-1 text-sm font-bold text-[var(--color-danger)]">{student.accuracy}%</span></li>)}</ul> : <div className="mt-4 rounded-2xl bg-white/60 p-5"><p className="font-bold text-[var(--color-success)]">Walang agarang abisong nangangailangan ng tutok.</p><p className="text-sm text-[var(--color-text-muted)]">Patuloy na subaybayan ang pagganap ng klase.</p></div>}
        </section>

        <section aria-labelledby="insight-title" className="rounded-3xl border p-5 shadow-card sm:p-6" style={cardStyle('--color-brand-sage', 8, 28)}><p className="text-xs font-bold tracking-[0.1em] text-[var(--color-brand-sage)] uppercase">Pananaw sa klase</p><h2 id="insight-title" className="text-xl font-bold">Buod ng Pagbasa</h2><div className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-white/65 p-4"><p className="text-2xl font-bold text-[var(--color-brand-sage)]">{rows.filter((row) => row.accuracy >= 80).length}</p><p className="text-xs font-bold text-[var(--color-text-muted)]">80%+ kawastuhan</p></div><div className="rounded-2xl bg-white/65 p-4"><p className="text-2xl font-bold text-[var(--color-brand-coral)]">{rows.filter((row) => row.streak >= 3).length}</p><p className="text-xs font-bold text-[var(--color-text-muted)]">3+ araw na sunod-sunod</p></div></div><p className="mt-4 text-sm leading-relaxed text-[var(--color-text-muted)]">{activeRows.length ? `Ang kasalukuyang karaniwang marka ng klase ay ${classAverage}%. Gamitin ang pagsusuri upang makita ang indibidwal na pagganap at susunod na pagtutuunan.` : 'Wala pang sapat na datos sa pagsasanay upang makagawa ng pananaw sa klase.'}</p></section>
      </div>

      <section aria-labelledby="recent-title" className="rounded-3xl border p-5 shadow-card sm:p-6" style={cardStyle('--color-brand-teal', 7, 26)}><div className="flex items-center justify-between"><h2 id="recent-title" className="text-xl font-bold">Kamakailang Aktibidad</h2><span className="text-xs font-bold text-[var(--color-text-muted)]">Pinakabagong pagsasanay</span></div>{recent?.length ? <div className="mt-4 grid grid-cols-1 gap-2 md:grid-cols-2">{recent.map((session, index) => <div key={`${session.created_at}-${index}`} className="flex min-w-0 items-center gap-3 rounded-2xl border border-white/70 bg-white/65 p-3"><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl font-bold ${session.accuracy_percentage >= 80 ? 'bg-[var(--color-success-soft)] text-[var(--color-success)]' : 'bg-[var(--color-warning-soft)] text-[var(--color-warning-text)]'}`}><AppIcon name={session.accuracy_percentage >= 80 ? '✓' : '⚠'} className="h-5 w-5" /></span><div className="min-w-0 flex-1"><p className="truncate font-bold">{nameFor(session.student_id)} · {session.word}</p><time dateTime={session.created_at} className="text-xs text-[var(--color-text-muted)]">{new Date(session.created_at).toLocaleString('fil-PH', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}</time></div><span className="font-bold">{session.accuracy_percentage}%</span></div>)}</div> : <p className="mt-4 text-[var(--color-text-muted)]">Wala pang kamakailang aktibidad.</p>}</section>
