    ...
    <div className="flex min-w-0 flex-col gap-6">
      ...
      {chartData.length > 0 && <>
        ...
        <div className="grid min-w-0 grid-cols-1 gap-5 lg:grid-cols-2">
          {[{ title: 'Kawastuhan ng Bawat Mag-aaral', desc: 'Mas mataas ay mas mahusay; target ang 80% pataas.', dataKey: 'accuracy', color: 'var(--color-brand-teal)', domain: [0, 100] as [number, number] }, { title: 'Natapos na Gawain', desc: 'Dami ng natapos na gawaing pagbasa.', dataKey: 'activities', color: 'var(--color-brand-sage)', domain: undefined }].map((chart) => <section key={chart.dataKey} className="min-w-0 rounded-3xl border p-4 shadow-card sm:p-5" style={cardStyle(chart.dataKey === 'accuracy' ? '--color-brand-teal' : '--color-brand-sage', 6, 24)}><h2 className="text-lg font-bold">{chart.title}</h2><p className="text-xs text-[var(--color-text-muted)]">{chart.desc}</p><div className="mt-3 h-72 min-w-0"><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} margin={{ top: 8, right: 6, left: -22, bottom: 0 }}><CartesianGrid strokeDasharray="4 4" vertical={false} stroke="var(--color-border)" /><XAxis dataKey="name" tick={{ fontSize: 10 }} tickLine={false} axisLine={false} interval={0} tickFormatter={(value: string) => value.split(' ')[0]} /><YAxis domain={chart.domain} tick={{ fontSize: 10 }} tickLine={false} axisLine={false} /><Tooltip contentStyle={{ borderRadius: 16, border: '1px solid var(--color-border)' }} /><Bar dataKey={chart.dataKey} fill={chart.color} radius={[8, 8, 0, 0]} /></BarChart></ResponsiveContainer></div></section>)}
        </div>
