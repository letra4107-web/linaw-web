...
export function SlowTTSButton({ text }: { text: string }) {
  ...
  return <button type="button" disabled={playing} onClick={async () => { setPlaying(true); try { await playTts(text, 0.3, 'fil-PH'); } finally { setPlaying(false); } }} className="inline-flex min-h-11 items-center gap-2 rounded-full border border-[var(--color-border)] bg-white/70 px-3 py-1.5 text-sm hover:border-[var(--color-primary)] disabled:opacity-60">
